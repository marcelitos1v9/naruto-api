...
const KaraMembers = () => {
  ...
  const membrosPorPagina = 9; // Você pode ajustar este número conforme necessário

  useEffect(() => {
    const buscarMembrosKara = async () => {
      try {
        setCarregando(true);
        const resposta = await axios.get(`https://narutodb.xyz/api/kara?page=${paginaAtual}&limit=${membrosPorPagina}`);
        setMembros(resposta.data.kara);
        setTotalPaginas(Math.ceil(resposta.data.totalKara / membrosPorPagina));
        setCarregando(false);
      } catch (error) {
        setErro('Ocorreu um erro ao carregar os membros da Kara.');
        setCarregando(false);
      }
    };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4 text-orange-900">Membros da Kara</h1>
